...
export default function RatingStars({ value, showLabel = false, stacked = false, className = "" }) {
  ...
  return (
    <span
      role="img"
      aria-label={label}
      className={`inline-flex ${stacked ? "flex-col items-start gap-0.5" : "items-center gap-1.5"} ${className}`}
    >
      <span className="whitespace-nowrap leading-none text-amber-500" aria-hidden="true">
        {"★".repeat(value)}<span className="text-slate-300">{"★".repeat(5 - value)}</span>
      </span>
      {showLabel && (
        <span aria-hidden="true" className={`whitespace-nowrap text-slate-500 ${stacked ? "text-[11px]" : ""}`}>
